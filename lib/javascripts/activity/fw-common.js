...
$(function () {
    $.get(API_PATH + "activity/v1/userState.json", {}, function (data) {
        if (data.code == 10000) {
            var m = data.data;
            var avatar = m.sex == 1 ?
            location.protocol + '//www.gongchangp2p.cn/img/man.png' :
            location.protocol + '//www.gongchangp2p.cn/img/woman.png';
...
$UserReady(function (is_login, user) {
    ...
    $(".nav-link-mall").attr("href", "http://passport.9888keji.com/passport/login?sourceSite=dgsc&service=http://www.dougemall.com/")

    $("#header-username").html('<a href="http://www.9888keji.com/account/myHome.shtml">' + user.username + '</a>');
    $("#header-realname").html('<a href="http://www.9888keji.com/account/myHome.shtml">' + user.realname + '</a>');
